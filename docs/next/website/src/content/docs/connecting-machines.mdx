...
With `ForwardAgent` enabled in your SSH config, the remote Herdr server publishes a stable `SSH_AUTH_SOCK` to panes and retargets it on every connection. Agent forwarding keeps working after reconnects, including in panes that were already open.
